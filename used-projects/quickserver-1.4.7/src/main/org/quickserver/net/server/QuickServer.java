   ...
   Eg:
	...
	private void processServerHooks(int event) {
		...
		while(iterator.hasNext()) {
			...
			try	{
				...
				result = false;
				logger.warning("Error invoking "+hooktype+" hook ["+
					serverHook.getClass().getName()+"]: " + e.getMessage());
			}
			logger.fine("Invoked "+hooktype+" hook ["+
				serverHook.getClass().getName()+"] was: "+result);
		}
	}
	...
	public Object clone() {
		...
		try {
			...
			logger.warning("Error cloning : "+e);//should not happ
		}
		return object;
	}
